resource "kubectl_manifest" "ec2_node_class" {
  ...
  yaml_body = templatefile("${path.module}/modules/kubectl_karpenter/ec2_node_class.yml", {
    NAME              = var.karpenter_capacity[count.index].name
    INSTANCE_PROFILE  = var.aws_nodes_instance_profile_name
    AMI_ID            = var.aws_eks_amis[count.index].value
...
resource "kubectl_manifest" "nodepool" {
  count = length(var.karpenter_capacity)
  yaml_body = templatefile("${path.module}/modules/kubectl_karpenter/nodepool.yml", {
    NAME               = var.karpenter_capacity[count.index].name
